...
- **QR en cada liga**: junto a cada 🔗 hay ▦ QR que llena la pantalla para escanear (barra del tablero y cada familia en la app; anfitrión, tarjetas, mesas, talento y proveedores en el admin).
- **Historial por tandas**: la columna Historial de Pedidos pinta 60 tarjetas y un botón "Mostrar 60 más".
...
| 📈 **Data Room** | Tablero de dirección en 12 carpetas, cada una una pregunta de director financiero contestada con números, la gráfica correcta (líneas, barras, apiladas, dona, mapa de calor) y una frase narrada: Hoy · Ventas y proyección (tendencia semanal, cierre de mes, ritmo anual, banda ±1σ) · Sucursales · Producto y margen (receta vs precio, agotados, acompañamientos) · Zonas y dónde crecer (mapa Leaflet con pedidos, calor y manchas de demanda a >2.5 km de toda sucursal; geocodificación con Nominatim guardada en `ranitas/geo`) · Clientes (nuevos/recurrentes, retención, valor anual, enfriados con WhatsApp) · Cobranza (antigüedad, métodos, recordatorios) · Operación (tiempos por etapa, a tiempo, capacidad en hora pico, cocineros, repartidores) · Insumos (kg por guiso, food cost, compra de la semana) · Equipo y talento · Decisiones (reglas ordenadas por urgencia) · Datos (CSV). Periodo 7/28/90/365 y filtro por sucursal. | Dueño, dirección |
...
| 👕 **Uniformes** | Ficha por empleado (puesto, sucursal, dirección, altura, peso, cintura, pecho, cadera, calzado, corte, ajuste). Talla sugerida por prenda del juego según el puesto (pecho → XS…4XL con corrimiento por corte y ajuste; cintura → pulgadas pares o tallas dama, largo por altura; mandil por altura; calzado MX). Un clic crea la solicitud `ranitas/solicitudes/<id>` por aprobar; Compras → 📨 Solicitudes aprueba, consolida por prenda y talla, manda WhatsApp al proveedor con rubro Uniformes y marca entregada. Fichas en `ranitas/empleados/<id>` (id = usuario o `e-…`). La ven todos los usuarios. | Todos |
| 🌟 **Talento** | Reclutamiento: sitio público `/talento` con manifiesto, organigrama (Central · CEDIS de ciudad · Frentes, 14 puestos), perfil de cada puesto (qué harás, qué buscamos, un día, requisitos) y postulación por ciudad y nivel (aprendiz, junior, senior, líder) con 3 preguntas cerradas calificadas solas y 6 abiertas para ver el razonamiento. En el admin: base de candidatos con filtros, calificación C · B · A · AA · AAA, estados (nuevo, preseleccionado, seleccionado, contratado, descartado), notas, WhatsApp y vacantes abiertas por ciudad y puesto. Postulaciones siempre abiertas. | Dueño, dirección de ciudad | Ciudades: el formulario acepta cualquier ciudad de México (32 estados, catálogo `MX_CIUDADES` inlineado en talento, proveedores y admin); en el admin se **abren** ciudades (`ranitas/ciudades/<id>`) y "Todas" muestra solo las abiertas; las ciudades con candidatos sin abrir aparecen como "Ya hay gente esperando en…" con botón para abrirlas. Candidatos y postulaciones guardan `ciudad` (id), `ciudadN` y `estadoMx`.
